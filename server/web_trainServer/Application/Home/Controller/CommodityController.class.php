...
class CommodityController extends CommonController{
    ...
    public function editcommodity($comm_id = -1){
        $Food_Db = M('commodity');
    ...
    public function to_editcommodity($comm_id = -1){
        $Food_Db  =  M('commodity');
        $where    =  'Comm_id ='.$comm_id;
        ...
        if($result){
            $this->success('修改成功',U('home/commodity/commodity'));
        }else{
            $this->error('修改失败或数据没有修改');
        }
    }

    public function deletecommodity($comm_id = -1){
        ...
        $url     =  '.'.C('DEL_COMM_IMG').$result[0]['comm_picture_url'];
        ...
        if($del_result){
            delfile($url);
            $this->success("删除数据成功",U('home/commodity/commodity'));
        }else{
            $this->error("删除失败");
        }
    }

    public function findcommodity($find = '!@#'){
    ...
    public function commentcommodity($comm_id= '!@#'){
        $where = 'comm_id ='.$comm_id;
    ...
    public function editcommodityimg(){
         $comm_id = I('comm_id');
        $where = "comm_id =".$comm_id;
        $food  = M('commodity');
        ...
        if(!$info){
            ...
            if($result){
                $this->success('图片修改成功',U('home/commodity/commodity'));
            }else
            {
                $this->error('修改失败');
            }
